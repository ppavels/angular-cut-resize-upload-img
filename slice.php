...
var cusid_ele = document.getElementsByTagName('ul');
for (var i = 0; i < cusid_ele.length; i++) {
    var item = cusid_ele[i];  
    if (!item.hasChildNodes() ) {
    item.className = item.className + "x";
    item.parentNode.className = item.parentNode.className + " alert-success";
    }
}
        ...
        // if successful, bind success message to message
                                    ...
                                    var myUL = document.getElementById(data.full);
                                    if(myUL) myUL.className = myUL.className + " alert-success";
...
//                echo '<tr><td>' . $item . '</td><td></td><td></td></tr>';
                ...
                $compl = '';
                if (file_exists($path .'/'.$item. "/.fullfolder")&&file_exists($path .'/'.$item. "/normal")) $compl='alert-success';
                // month
             $filesy=scandir($path .'/'.$item);
            $cud_files = count($filesy);
            if (in_array('..', $filesy)) $cud_files=$cud_files-2;
  //          if ($cud_files==0) {$compl='alert-success'; $inactive='1';};
                    echo '
                            <li class="dropdown-submenu '. $compl .'" id="'.$path .'/'.$item.'">
...
//                echo '<tr><td></td><td>' . $item . '</td><td></td><td></td></tr>';
      ...
      //      
            ...
            if ($curr_width == $width && $curr_height == $height) {
                $to_cut++;
                // file
                echo   '<li class="'.$tr_class.'" id="'.$item.'">'.$item . '</li>';
